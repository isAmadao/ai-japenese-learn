...
const word = ref<WordDetailType | null>(null)
...
async function loadWord() {
  ...
  try {
    ...
    error.value = e?.response?.data?.detail || '加载单词详情失败'
    word.value = null
  } finally {
    loading.value = false
  }
    ...
    <template v-else-if="word">
      <div class="detail-card card">
        <div class="word-header">
          <div>
            <h1 class="japanese">{{ word.japanese }}</h1>
        ...
        <div class="sentences-section">
          <h3>📖 例句 <span class="hint">（点击 🔊 可朗读）</span></h3>
      ...
      <!-- Related Articles -->
      <div class="articles-section card" v-if="word.articles && word.articles.length > 0">
        <h3>📄 相关文章</h3>
